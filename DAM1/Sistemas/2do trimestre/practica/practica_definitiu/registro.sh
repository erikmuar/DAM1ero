...
while [ $break -eq -1 ]; do
    ...
    if [[ -z $estar ]]; then
        ...
        if [[ ! -f registroEntradaSalida.txt ]]; then
            > registroEntradaSalida.txt
        fi
        ...
        existe=$(awk -v var="$id" -v fecha="$fecha" '$1==var && $3==fecha {print $1, $2, $3, $4, $5}' registroEntradaSalida.txt)

        if [[ -z $existe ]]; then
            # Registrar entrada
            nombre=$(echo "$estar" | awk '{print $2}')
            echo "$id $nombre $fecha $hora" >> registroEntradaSalida.txt
            ...
            if [[ "$columnas" -eq 4 ]]; then
                ...
                done < registroEntradaSalida.txt > registroEntradaSalida.tmp && mv registroEntradaSalida.tmp registroEntradaSalida.txt
