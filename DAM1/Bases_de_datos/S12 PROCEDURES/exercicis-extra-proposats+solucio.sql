...
-- opció 1 amb control de overflow de bucle
DELIMITER $$
DROP PROCEDURE IF EXISTS nSeafoodSearcher $$
CREATE PROCEDURE nSeafoodSearcher ()
BEGIN
...
categories_loop: LOOP
	SET contador = contador + 1;
	IF done=1 OR contador > (SELECT COUNT(*) FROM categories) THEN
		LEAVE categories_loop;
	END IF;
	
	SET @vCategoryName = (SELECT CategoryName FROM Categories WHERE CategoryID = contador);
	IF @vCategoryName IS NULL THEN
		LEAVE categories_loop;
	END IF;
	IF @vCategoryName LIKE 'Seafood' THEN
		SELECT CONCAT('Categoria Seafood trobada amb id:', contador); 
...
BEGIN
DECLARE vID int;
DECLARE vCategoryName varchar(255);
DECLARE done int DEFAULT 0;
DECLARE cur1 CURSOR FOR SELECT CategoryID, CategoryName FROM Categories;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1;

categories_loop: LOOP
	FETCH cur1 INTO vID, vCategoryName;
	IF done=1 THEN
		LEAVE categories_loop;
	END IF;
	
	IF vCategoryName LIKE 'Seafood' THEN
		SELECT CONCAT('Categoria Seafood trobada amb id:', vID); 
		SET done=1;
	END IF;
END LOOP categories_loop;
